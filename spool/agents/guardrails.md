...
- **shared** — Before proposing a memory framework, embedding index, vector store, plan-DAG, sub-agent runtime, or other non-Markdown infrastructure for spool, re-read `spool/gh/issue/archive/2-perfect-memory-research/analysis.md`. These categories were surveyed across ~17 dev-agent tools and rejected with reasoning. Re-litigating without new evidence is the failure mode (#2).
